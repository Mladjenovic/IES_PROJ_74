...
namespace TelventDMS.Services.NetworkModelService.TestClient.Tests
{
	...
	{			
        ...
        {
            string message = "Getting processes values method started.";
            Console.WriteLine(message);
            CommonTrace.WriteTrace(CommonTrace.TraceError, message);


            ModelCode type = ModelCode.PROCESS;
            int numberOfResources = 2;
            XmlTextWriter writer = null;

            try
            {
                List<ModelCode> propertyID = modelResourcesDesc.GetAllPropertyIds(type);

                int iteratorId = GdaQueryProxy.GetExtentValues(type, propertyID);
                int resourcesLeft = GdaQueryProxy.IteratorResourcesLeft(iteratorId);

                writer = new XmlTextWriter(Config.Instance.ResultDirecotry + "\\Processes.xml", Encoding.Unicode);
                writer.Formatting = Formatting.Indented;

                while (resourcesLeft > 0)
                {
                    List<ResourceDescription> resourceDesc = GdaQueryProxy.IteratorNext(numberOfResources, iteratorId);

                    for (int i = 0; i < resourceDesc.Count; i++)
                    {
                        resourceDesc[i].ExportToXml(writer);
                        writer.Flush();
                    }
                ...
                Console.WriteLine("Getting Processes successfully finished!");
            }
            catch (Exception e)
            {
        ...
        public void GetMarketDocumentsFromProcessWithLowestGID()
        {
            string message = "Getting Market document with lowest gid method started.";
            Console.WriteLine(message);
            CommonTrace.WriteTrace(CommonTrace.TraceError, message);
            ...
            try
            {
                ...
                int iteratorId = GdaQueryProxy.GetExtentValues(type, properties);
                int resourcesLeft = GdaQueryProxy.IteratorResourcesLeft(iteratorId);


                writer = new XmlTextWriter(Config.Instance.ResultDirecotry + "\\MarketDocuments.xml", Encoding.Unicode);
                writer.Formatting = Formatting.Indented;
            ...
            association.Type = ModelCode.MARKETDOCUMENT;
            association.PropertyId = ModelCode.PROCESS_MARKETDOCUMENTS;
            ...
            try
            {
                ...
                int iteratorId = GdaQueryProxy.GetRelatedValues(GID, properties, association);
                int resourcesLeft = GdaQueryProxy.IteratorResourcesLeft(iteratorId);
                ...
                Console.WriteLine("Getting MarketDocuments of Process with lowest GID successfully finished!!");
            }
            catch (Exception e)
            {
                Console.WriteLine(e.Message);
