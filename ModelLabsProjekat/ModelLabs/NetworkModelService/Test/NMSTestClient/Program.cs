...
namespace FTN.Services.NetworkModelService.TestClient
{
	public class Program
	{
			...
			try
			{
				...
				do
				{
                    ...
                    else if (str == "5")
                    {
                        try
                        {
                            tgda.GetAllProcesses();
                        }
                        catch (Exception ex)
                        {
                            message = string.Format("Test Get all processes method failed. {0}", ex.Message);
                            Console.WriteLine(message);
                            CommonTrace.WriteTrace(CommonTrace.TraceError, message);
                        }
                    }
                    else if (str == "6")
                    {
                        try
                        {
                            tgda.GetMarketDocumentsFromProcessWithLowestGID();
                        }
                        catch (Exception ex)
                        {
                            message = string.Format("Test Aditional method failed. {0}", ex.Message);
                            Console.WriteLine(message);
                            CommonTrace.WriteTrace(CommonTrace.TraceError, message);
                        }
                    }
		...
		private static void PrintMenu()
		{
            ...
            Console.WriteLine("\t5) Get all processes");
            Console.WriteLine("\t6) Get Market documents related to Process with lowest GID");
